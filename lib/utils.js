 ...
 * @param   {string}   proto_username Expected username
 * @param   {string}   proto_password Expected password
 * @returns {function} Express 4 middleware requiring the given credentials
 */

exports.basicAuth = function (username, password, proto_username, proto_password) {
  return function (req, res, next) {
    if (!username || !password || !proto_username || !proto_password) {
    ...
    if (!user || user.name !== username || user.pass !== password || user.name !== proto_username || user.pass !== proto_password) {
